...
const createSupplierCategories = async () => {
  ...
  supplierCategories.value.push(data);
  onReset();
}
const onReset = () => {
  name.value = null;
    ...
    <q-form>
      <q-card class="full-width" style="max-width: 450px;">
        <q-card-section
            ...
            class="text-center">
          <q-card-section>
            <h4>Crear Categoria Suplidor</h4>
          </q-card-section>
            ...
            <div class="form-column">
              <q-card-section>
                <q-input
                    filled
                    v-model="name"
                    label="Categoria Suplidor"
                    hint="Entidad"
                    lazy-rules
                    :rules="[ val => val && val.isEmpty || 'Debe escribir una categoria de suplidor']"
                />
              </q-card-section>
            </div>
            <div class="divButtons">
              <q-btn color="primary" @click="createSupplierCategories">Create</q-btn>
